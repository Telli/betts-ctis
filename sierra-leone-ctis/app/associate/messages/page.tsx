...
export default function AssociateMessagesPage() {
  const { user } = useAuth()
  ...
  const loadClients = async () => {
    try {
      // This would typically come from an associate service
      // For now, we'll use a placeholder
      setClients([
        { id: "client-1", name: "ABC Company Ltd", clientNumber: "CL001" },
        { id: "client-2", name: "XYZ Enterprises", clientNumber: "CL002" },
        { id: "client-3", name: "Sierra Leone Traders", clientNumber: "CL003" }
      ])
    } catch (error) {
  ...
  const handleSendMessage = async () => {
    try {
      const messageData: SendMessageDto = {
        ...
        clientId: composeForm.clientId ? parseInt(composeForm.clientId) : undefined,
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-sierra-orange-800">Client Messages</h1>
          <p className="text-sierra-orange-600">Communicate with your clients</p>
        </div>
          ...
          <DialogContent className="max-w-2xl">
            ...
            <div className="space-y-4">
              <div>
                ...
                <Select value={composeForm.clientId} onValueChange={(value) => {
                  setComposeForm(prev => ({ ...prev, clientId: value }))
                  // Find the client and set recipientId to the client's user ID
                  const client = clients.find(c => c.id === value)
                  if (client) {
                    setComposeForm(prev => ({ ...prev, recipientId: `client-${client.id}` }))
                  }
                }}>
                  <SelectTrigger>
